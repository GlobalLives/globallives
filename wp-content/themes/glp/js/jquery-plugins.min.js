!function($, undefined) {
    ...
    if ($.support.touch = "ontouchend" in document, $.support.touch) {
        ...
        mouseProto._touchStart = function(event) {
            var self = this;
            !touchHandled && self._mouseCapture(event.originalEvent.changedTouches[0]) && (touchHandled = !0, 
            self._touchMoved = !1, simulateMouseEvent(event, "mouseover"), simulateMouseEvent(event, "mousemove"), 
            simulateMouseEvent(event, "mousedown"));
        }, mouseProto._touchMove = function(event) {
            touchHandled && (this._touchMoved = !0, simulateMouseEvent(event, "mousemove"));
        }, mouseProto._touchEnd = function(event) {
            touchHandled && (simulateMouseEvent(event, "mouseup"), simulateMouseEvent(event, "mouseout"), 
            this._touchMoved || simulateMouseEvent(event, "click"), touchHandled = !1);
    ...
    function go(els, opts, manual, fwd) {
        function queueNext(opts) {
            opts.timeout && (p.cycleTimeout = setTimeout(function() {
                go(els, opts, 0, !opts.rev);
            }, opts.timeout));
        }
        if (!opts.busy) {
            var p = els[0].parentNode, curr = els[opts.currSlide], next = els[opts.nextSlide];
            if (0 !== p.cycleTimeout || manual) if (manual || !p.cyclePause) {
                opts.before.length && $.each(opts.before, function(i, o) {
                    o.apply(next, [ curr, next, opts, fwd ]);
                });
                var after = function() {
                    msie && this.style.removeAttribute("filter"), $.each(opts.after, function(i, o) {
                        o.apply(next, [ curr, next, opts, fwd ]);
                    }), queueNext(opts);
                };
                opts.nextSlide != opts.currSlide && (opts.busy = 1, $.fn.cycle.custom(curr, next, opts, after));
    ...
    function advance(els, opts, val) {
        ...
        return timeout && (clearTimeout(timeout), p.cycleTimeout = 0), opts.nextSlide = opts.currSlide + val, 
        opts.nextSlide < 0 ? opts.nextSlide = els.length - 1 : opts.nextSlide >= els.length && (opts.nextSlide = 0), 
        go(els, opts, 1, val >= 0), !1;
    }
    var ver = "Lite-1.7", msie = /MSIE/.test(navigator.userAgent);
    $.fn.cycle = function(options) {
        return this.each(function() {
            options = options || {}, this.cycleTimeout && clearTimeout(this.cycleTimeout), this.cycleTimeout = 0, 
            this.cyclePause = 0;
            var $cont = $(this), $slides = options.slideExpr ? $(options.slideExpr, this) : $cont.children(), els = $slides.get();
            if (els.length < 2) return void (window.console && console.log("terminating; too few slides: " + els.length));
            var opts = $.extend({}, $.fn.cycle.defaults, options || {}, $.metadata ? $cont.metadata() : $.meta ? $cont.data() : {}), meta = $.isFunction($cont.data) ? $cont.data(opts.metaAttr) : null;
            meta && (opts = $.extend(opts, meta)), opts.before = opts.before ? [ opts.before ] : [], 
            opts.after = opts.after ? [ opts.after ] : [], opts.after.unshift(function() {
                opts.busy = 0;
            });
            var cls = this.className;
            opts.width = parseInt((cls.match(/w:(\d+)/) || [])[1], 10) || opts.width, opts.height = parseInt((cls.match(/h:(\d+)/) || [])[1], 10) || opts.height, 
            opts.timeout = parseInt((cls.match(/t:(\d+)/) || [])[1], 10) || opts.timeout, "static" == $cont.css("position") && $cont.css("position", "relative"), 
            opts.width && $cont.width(opts.width), opts.height && "auto" != opts.height && $cont.height(opts.height);
            var first = 0;
            $slides.css({
                position: "absolute",
                top: 0
            ...
            }), $(els[first]).css("opacity", 1).show(), msie && els[first].style.removeAttribute("filter"), 
            opts.fit && opts.width && $slides.width(opts.width), opts.fit && opts.height && "auto" != opts.height && $slides.height(opts.height), 
            opts.pause && $cont.hover(function() {
                this.cyclePause = 1;
            }, function() {
                this.cyclePause = 0;
            });
            var txFn = $.fn.cycle.transitions[opts.fx];
            if (txFn && txFn($cont, $slides, opts), $slides.each(function() {
                var $el = $(this);
                this.cycleH = opts.fit && opts.height ? opts.height : $el.height(), this.cycleW = opts.fit && opts.width ? opts.width : $el.width();
            }), opts.cssFirst && $($slides[first]).css(opts.cssFirst), opts.timeout) for (opts.speed.constructor == String && (opts.speed = {
                slow: 600,
                fast: 200
            }[opts.speed] || 400), opts.sync || (opts.speed = opts.speed / 2); opts.timeout - opts.speed < 250; ) opts.timeout += opts.speed;
            opts.speedIn = opts.speed, opts.speedOut = opts.speed, opts.slideCount = els.length, 
            opts.currSlide = first, opts.nextSlide = 1;
            var e0 = $slides[first];
            opts.before.length && opts.before[0].apply(e0, [ e0, e0, opts, !0 ]), opts.after.length > 1 && opts.after[1].apply(e0, [ e0, e0, opts, !0 ]), 
            opts.click && !opts.next && (opts.next = opts.click), opts.next && $(opts.next).unbind("click.cycle").bind("click.cycle", function() {
                return advance(els, opts, opts.rev ? -1 : 1);
            }), opts.prev && $(opts.prev).unbind("click.cycle").bind("click.cycle", function() {
                return advance(els, opts, opts.rev ? 1 : -1);
            }), opts.timeout && (this.cycleTimeout = setTimeout(function() {
                go(els, opts, 0, !opts.rev);
        ...
        fade: function($cont, $slides, opts) {
            $slides.not(":eq(0)").hide(), opts.cssBefore = {
                opacity: 0,
                display: "block"
            }, opts.cssAfter = {
                display: "none"
        ...
        fadeout: function($cont, $slides, opts) {
            opts.before.push(function(curr, next, opts, fwd) {
                $(curr).css("zIndex", opts.slideCount + (fwd === !0 ? 1 : 0)), $(next).css("zIndex", opts.slideCount + (fwd === !0 ? 0 : 1));
            }), $slides.not(":eq(0)").hide(), opts.cssBefore = {
                opacity: 1,
                display: "block",
                zIndex: 1
